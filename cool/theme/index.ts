...
const STORAGE_KEY_IS_AUTO = "app_theme_auto";
 ...
 */
const getTheme = () => {
	// 优先从本地存储读取用户保存的自动主题状态
	const savedIsAutoValue = storage.get(STORAGE_KEY_IS_AUTO);
	if (savedIsAutoValue != null) {
		// 处理从存储中读取的值（可能是字符串 "true"/"false" 或布尔值）
		if (typeof savedIsAutoValue === "boolean") {
			isAuto.value = savedIsAutoValue;
		} else if (typeof savedIsAutoValue === "string") {
			isAuto.value = savedIsAutoValue === "true";
		}
	}
	...
	if (isAuto.value) {
		...
		return osTheme as Theme;
		// #endif
		
		// #ifdef H5 || MP
		const hostTheme = uni.getAppBaseInfo().hostTheme;
		return (hostTheme != null && hostTheme != "" ? hostTheme : "light") as Theme;
		// #endif
	}

	// 非自动模式，优先从本地存储读取用户保存的主题
	const savedTheme = storage.get(STORAGE_KEY_THEME) as string | null;
...
export const setIsAuto = () => {
	...
	// 持久化自动主题状态
	storage.set(STORAGE_KEY_IS_AUTO, isAuto.value, 0);
	...
	if (isAuto.value) {
		...
		// 清除保存的主题，因为使用自动模式
		storage.remove(STORAGE_KEY_THEME);
	} else {
		// 关闭自动，使用当前 theme
...
export const setTheme = (value: Theme) => {
	...
	// 持久化主题设置
	storage.set(STORAGE_KEY_THEME, value, 0);
	storage.set(STORAGE_KEY_IS_AUTO, false, 0);

	// 立即更新主题值，不等待异步操作，避免闪烁
	theme.value = value;
...
export const applyTheme = () => {
	...
	// 立即应用主题到系统（同步设置，避免闪烁）
	uni.setAppTheme({
		theme: isAuto.value ? "auto" : currentTheme
	});
	// 立即更新主题值，不等待成功回调
	if (!isAuto.value) {
