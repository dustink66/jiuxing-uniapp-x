...
export class Activity {
	...
	setActivities(data: any) {
		...
		// 使用 parse 函数转换每个元素，在 Android 平台上会自动处理 UTSJSONObject
		this.activities.value = data.map((item: any) => parse<ActivityInfo>(item)!).filter((item: ActivityInfo | null) => item != null) as ActivityInfo[];
	}
	...
	setMyActivities(data: any) {
		...
		// 使用 parse 函数转换每个元素，在 Android 平台上会自动处理 UTSJSONObject
		this.myActivities.value = data.map((item: any) => parse<ActivityInfo>(item)!).filter((item: ActivityInfo | null) => item != null) as ActivityInfo[];
	}
	...
	setCurrentActivity(data: any) {
		...
		// 使用 parse 函数转换数据，在 Android 平台上会自动处理 UTSJSONObject
		const parsed = parse<ActivityInfo>(data);
		if (parsed != null) {
			this.currentActivity.value = parsed;
		} else {
			this.currentActivity.value = null;
		}
	}
