...
export class Router {
	...
	back(options: BackOptions | null = null) {
		if (this.isFirstPage()) {
			...
			if (this.eventsMap.beforeEach != null) {
				...
				// 当前页
				const from = last(pages);

				// 上一页
				const to = nth(pages, -delta - 1);
				...
				if (to != null) {
					...
					console.error("[router] found to page is null, delta:", delta, "pages:", pages.length);
					// 如果找不到目标页面，直接执行跳转（使用 uni.navigateBack）
					next();
				}
			} else {
				next();
	...
	nextLogin() {
		...
		// 未找到登录页，或页面栈计算有问题，则跳回首页
		if (index < 0) {
			this.home();
		} else {
			const delta = pages.length - index;
			// 验证 delta 是否有效
